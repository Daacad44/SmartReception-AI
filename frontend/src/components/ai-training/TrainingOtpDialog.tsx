...
import type { TrainingVerificationRequest } from '@/lib/ai-training-center-types';
...
interface TrainingOtpDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  verification: TrainingVerificationRequest | null;
  onVerified?: () => void;
}

export function TrainingOtpDialog({
  open,
  onOpenChange,
  verification,
  onVerified,
}: TrainingOtpDialogProps) {
  ...
  const confirm = useMutation({
    ...
    onSuccess: () => {
      setSuccess(true);
      toast.success('Training operation started');
      onVerified?.();
      setTimeout(() => {
        setSuccess(false);
        setCode('');
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            A 6-digit code was sent to your Super Admin email. Enter it to unlock{' '}
            <strong>{verification?.operationLabel ?? 'this training operation'}</strong>.
          </DialogDescription>
        </DialogHeader>
        <DialogBody>
          {success ? (
            <div className="flex flex-col items-center gap-3 py-8 text-center">
              ...
              <p className="text-sm text-muted-foreground">Training is starting automatically.</p>
            </div>
          ) : (
            <div className="space-y-3">
              <Label htmlFor="training-otp">6-digit code</Label>
              <Input
                id="training-otp"
                inputMode="numeric"
                maxLength={6}
