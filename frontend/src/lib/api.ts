...
import { readApiData, readSuccessBody } from '@/lib/api-response';
...
/** Message-only endpoints return { success: true, message } with no `data` payload. */
export function assertSuccess(response: AxiosResponse<ApiResponse>): void {
  readSuccessBody(response.data);
}
